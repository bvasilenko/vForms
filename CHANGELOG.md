...
- `@booga/vdsl` dependency. vForms never imported it — the package was declared
  in `dependencies` and listed in the tsup `external` array but unused. Its stale
  `^0.1.1` range also made consumers install a pre-fix vDsl needlessly.
...
- `@booga/vui` dependency raised to `^0.2.0`. vUi 0.2.0 resolves its color classes through `@booga/vtheme`'s semantic role contract, so form primitives render correctly once the consumer applies vTheme's Tailwind preset (`presets: [require("@booga/vtheme/preset")]`).
